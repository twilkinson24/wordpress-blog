...
if ( ! function_exists( '_s_posted_on_blog_roll' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function _s_posted_on_blog_roll() {
		$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
		if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
			$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
		}
	//	echo get_the_date( 'Y/m/d \a\t g:i A' );

		$time_string = sprintf(
			$time_string,
			esc_attr( get_the_date( 'm/d/Y \a\t g:i A' ) ),
			esc_html( get_the_date( 'm/d/Y \a\t g:i A' ) ),
			esc_attr( get_the_modified_date( 'm/d/Y \a\t g:i A' ) ),
			esc_html( get_the_modified_date( 'm/d/Y \a\t g:i A' ) )
		);
...
if ( ! function_exists( '_s_posted_by_blog' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function _s_posted_by_blog() {
		$byline = sprintf(
